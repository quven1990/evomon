...
export function generateLlmsTxt(): string {
  ...
  return lines(
    ...
    `- [Natures](${canonical("/natures")}): Full +10% / −10% nature table and reroll tips.`,
    `- [Traits](${canonical("/traits")}): Trait effects, rarity, and which pets can roll them.`,
    `- [Team Builder](${canonical("/team-builder")}): Plan a 5-pet party, check type coverage, share via URL.`,
    `- [Map Zones](${canonical("/map-zones")}): Island level ranges, hunt tips, and spawn lists.`,
  "",
    "## Guides",
    ...
    `- [Equipment](${canonical("/equipment")}): Silent Sands gear dungeons — Enhance vs Refine, Gear Station.`,
    `- [Items](${canonical("/items")}): Balls, evolution stones, reroll potions — what to spend first.`,
    `- [Level 30 Ascension](${canonical("/guides/level-30")}): Ultimate unlock and handoff to gear.`,
...
export function generateLlmsFullTxt(): string {
  ...
  return lines(
    ...
    `- Dex: ${canonical("/dex")}`,
    `- Natures: ${canonical("/natures")}`,
    `- Traits: ${canonical("/traits")}`,
    `- Map zones: ${canonical("/map-zones")}`,
